...
TWEET_FILE = os.path.join(CURRENT_DIR, 'Tweets/sampled.pickle')
...
def clean_tweets(filename):
	...
	bot_users = [user for user in botscores if botscores[user]['scores']['universal'] >= 4 or botscores[user]['scores']['english'] >= 4]
	print(f'Nuber of bot users: {len(bot_users)}')
	non_bots = english_tweets[~english_tweets.UserId.isin(bot_users)]
	bots = english_tweets[english_tweets.UserId.isin(bot_users)]
	print(f'{bots.shape[0]} bot tweets removed. {non_bots.shape[0]} tweets remain.')
	return non_bots, bots

def main():
	total_non_bots, total_bots = clean_tweets(TWEET_FILE)
	total_non_bots.to_pickle(os.path.join(CURRENT_DIR, 'Tweets/cleanNonBotTweets.pickle'))
	total_bots.to_pickle(os.path.join(CURRENT_DIR, 'Tweets/cleanBotTweets.pickle'))
