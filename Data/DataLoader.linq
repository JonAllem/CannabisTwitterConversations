...
void Main()
{
	...
	{
		var filename = $@"D:\Documents\GitHub\Cannabis\Data\Tweets\Tweets-{start.Day}-{start.Month}-{start.Year}.csv";
	...
	foreach (var (start, stop) in dateRanges)
	{
		var filename = $@"D:\Documents\GitHub\Cannabis\Data\Users\Users-{start.Day}-{start.Month}-{start.Year}.csv";
		Util.WriteCsv(Concatenate(GetUsers(start, stop)), filename);
		Console.WriteLine("Finished 1 user query.");
	}

	var files = Directory.GetFiles(@"D:\Documents\GitHub\Cannabis\Data\Users\", "Users-*.csv");
	...
	Util.WriteCsv(userDict, @"D:\Documents\GitHub\Cannabis\Data\Users\AllUsers.csv");
}

public List<string> GetKeywords()
{
	return new List<string> { "blunt", "bong", "budder", "cannabis", "cbd", "ganja", "hash", "hemp", "indica", "kush", "marijuana", "marihuana", "reefer", "sativa", "thc", "weed" };
}
